...
func (d *DeleteLoadbalancerInput) DeleteLoadbalancer(con aws.EstablishConnectionInput) ([]LoadBalanceDeleteResponse, error) {
	...
	lb_delete_status := make([]LoadBalanceDeleteResponse, 0)
	...
	case "application":
		...
		if d.LbNames != nil {
			for _, lb := range d.LbNames {
				...
				lb_delete_status = append(lb_delete_status, LoadBalanceDeleteResponse{LbDeleteStatus: "LoadBalancer deletion is successful", LbArn: lbarn.LbArns[0]})
			}
			return lb_delete_status, nil
		}
		...
		if d.LbArns != nil {
			...
			for _, lbarn := range d.LbArns {
				...
				lb_delete_status = append(lb_delete_status, LoadBalanceDeleteResponse{LbDeleteStatus: "LoadBalancer deletion is successful", LbArn: lbarn})
			}
			return lb_delete_status, nil
		}
	...
	case "classic":
		if d.LbNames != nil {
			...
			for _, lbname := range d.LbNames {
				del_err := elb.DeleteClassicLoadbalancer(
				...
				lb_delete_status = append(lb_delete_status, LoadBalanceDeleteResponse{LbDeleteStatus: "LoadBalancer deletion is successful", LbName: lbname})
			}
			return lb_delete_status, nil
		}
